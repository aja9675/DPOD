...
class LineMODDataset(Dataset):
    ...
    def __init__(self, root_dir, train_eval_dir, classes=None, transform=None):
        self.root_dir = root_dir
        self.transform = transform
        self.classes = classes
        self.list_all_images = load_obj(os.path.join(root_dir, "all_images_adr"))
        self.training_images_idx = load_obj(os.path.join(train_eval_dir, "train_images_indices"))
    ...
    def __getitem__(self, i):
        ...
        IDmask = cv2.imread(os.path.join(self.root_dir, label + "/ground_truth/IDmasks/color" +
                            str(idx) + ".png"), cv2.IMREAD_GRAYSCALE)
        Umask = cv2.imread(os.path.join(self.root_dir, label + "/ground_truth/Umasks/color" +
                           str(idx) + ".png"), cv2.IMREAD_GRAYSCALE)
        Vmask = cv2.imread(os.path.join(self.root_dir, label + "/ground_truth/Vmasks/color" +
                           str(idx) + ".png"), cv2.IMREAD_GRAYSCALE)
...
class PoseRefinerDataset(Dataset):
    ...
    def __init__(self, root_dir, train_eval_dir, classes=None, transform=None):
        self.root_dir = root_dir
        self.train_eval_dir = train_eval_dir
        self.transform = transform
        self.classes = classes
        self.list_all_images = load_obj(os.path.join(root_dir, "all_images_adr"))
        self.training_images_idx = load_obj(os.path.join(train_eval_dir, "train_images_indices"))
    ...
    def __getitem__(self, i):
        ...
        regex = re.compile(r'\d+')
        idx = regex.findall(os.path.split(img_adr)[1])[0]
        image = cv2.imread(os.path.join(self.train_eval_dir, label + '/pose_refinement/real/color' + str(idx) + ".png"))
        rendered = cv2.imread(
            os.path.join(self.train_eval_dir, label + '/pose_refinement/rendered/color' + str(idx) + ".png", cv2.IMREAD_GRAYSCALE))
        rendered = cv2.cvtColor(rendered.astype('uint8'), cv2.COLOR_GRAY2RGB)
        true_pose = get_rot_tra(os.path.join(self.root_dir, label + '/data/rot' + str(idx) + ".rot"),
                                os.path.join(self.root_dir, label + '/data/tra' + str(idx) + ".tra"))
        pred_pose_adr = os.path.join(self.train_eval_dir, label + '/predicted_pose/info_' + str(idx) + ".txt")
